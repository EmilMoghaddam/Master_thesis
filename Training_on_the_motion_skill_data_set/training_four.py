...
for model_class, pretrained, name in zip(m, is_pretrained, names):
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=os.path.join('checkpoint_path_four/', name), monitor="val step loss")
    logger = CSVLogger("training_logs_four", version=name)
