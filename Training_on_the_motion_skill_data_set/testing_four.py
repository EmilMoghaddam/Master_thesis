...
ANNOTATION_FILE = "csv_files/photos.csv"
photo_csv_file = "csv_files/photos.csv"
...
class NewRes(NeuralModel):
    ...
    def __init__(self, model_kwargs, lr, thresholds=8 * [0.5], pretrained=False):
...
class BeIT(NeuralModel):
    ...
    def __init__(self, model_kwargs, lr, thresholds=8 * [0.5], pretrained = True):
...
is_pretrained = [True, True, True, False]
m = [BeIT, ConvNext, NewRes, NewRes]

for model_class, pretrained, name in zip(m, is_pretrained, names):

    model = model_class(None, LEARNING_RATE, thresholds = 8*[0.5], pretrained= pretrained)
    ...
    pht = os.path.join('checkpoint_path_four/', name)
    file_name = os.listdir(pht)[0]
    cpt_pht = os.path.join(pht, file_name)

    logger = CSVLogger("logs_test_four", version=name)
    ...
    trainer.test(model, dataModule, ckpt_path=cpt_pht)